...
            return View();
        ...
        // --- TIER INTRO MANAGER ---
        // --- 2. ENTRY POINT: CATEGORY "MULTIPLE CHOICE" ---
        public IActionResult StartTierIntro(string id)
        {
            // id = "Beginner"
            ViewData["SceneNumber"] = 1;
            ViewData["QuizId"] = id;

            if (id == "Beginner")
            {
                // Load the specific view for Beginner
                return View("IntroBeginner");
            }
            else if (id == "Intermediate")
            {
                // Load the specific view for Intermediate
                return View("IntroIntermediate");
            }
            else if (id == "Professional")
            {
                // Load the specific view for Professional
                return View("IntroProfessional");
            }
            {
                // Load the specific view for Beginner
                return RedirectToAction("Index");
            }
            // Add other tiers here later (Intermediate, Professional)

            // Fallback
           
        }
        // --- VICTORY SCENE (End of Game) ---
        [AllowAnonymous]
        public IActionResult Victory(int scene = 1)
        {
            ViewData["SceneNumber"] = scene;
            return View(); // Loads Views/Quizzes/Victory.cshtml
        }
        // --- 3. NAVIGATION HANDLER FOR TIERS ---
        [AllowAnonymous]
        public IActionResult NavigateTierIntro(string viewName, int nextScene, string quizId)
        {
            ViewData["SceneNumber"] = nextScene;
